...
\begin{table*}[ht]\centering		
	...
	\resizebox{0.8\textwidth}{!}{ 	\renewcommand{\arraystretch}{.9}
		\begin{tabular}{@{}c <{\enspace}@{}lcccccc@{}}	\toprule
			...
			\cmidrule{2-8}	
			\multirow{2}{*}{\rotatebox{90}{GNN}}	& \textsc{Gin-$\varepsilon$}   &   \scriptsize $\pm \pm $       & $72.4$ \scriptsize $\pm 0.6\pm 4.0 $ &  $49.7$ \scriptsize $\pm 0.2 \pm  3.9$ & $79.0$ \scriptsize $\pm  0.5 \pm 1.9 $ &  $71.3$ \scriptsize $\pm 0.4 \pm 4.0$ & $90.0$ \scriptsize $\pm 0.6 \pm 1.7 $   \\
			& \textsc{Gin-$\varepsilon$-JK} &     \scriptsize $\pm \pm $       & $73.4$ \scriptsize $\pm 0.6\pm 4.4$ & $50.6$ \scriptsize $\pm 0.5 \pm 4.2 $ & $79.1$ \scriptsize $\pm  0.2 \pm 1.5 $ & $72.4$ \scriptsize $\pm 0.6 \pm 1.7 $ & $90.6$ \scriptsize $\pm 0.2 \pm  2.2$   \\
			\bottomrule
	\end{tabular}}
\end{table*}		
...
\xhdr{Baselines methods.} To provide meaningful baselines, we provide implementations of common graph kernel as well as GNNs baselines. We have implemented the \new{Weisfeiler-Lehman Subtree}~\cite{She+2011}, \new{Shortest-path}~\cite{Bor+2005}, \new{Graphlet}~\cite{She+2009}, \new{Weisfeiler-Lehman Optimal Assigment}~\cite{Kri+2016} kernel in \CPP and made them accessible through the Python interface of \textsc{TuDataset}, see the website for further details.  Moreover, all GNN architectures provided by \textsc{PyTorch Geometric} can be conveniently used as a baseline as well. 
...
\begin{table}[]\centering	
	...
	\resizebox{.75\columnwidth}{!}{\renewcommand{\arraystretch}{1.0}
		\begin{tabular}{lcc}	\toprule
			\multirow{3}{*}{\vspace*{4pt}\textbf{Method}}&\multicolumn{2}{c}{\textbf{Dataset}}\\\cmidrule{2-3}
			& {\textsc{Zinc}}     & {\textsc{alchemy}}   \\	\toprule
			\textsc{Gine-$\varepsilon$}    & 0.084                                                                                                                                                                                                                                     \scriptsize $\pm 0.004$   &                                                                                                                                                                                                                                0.103 {\scriptsize $\pm 0.001$} -2.956 {\scriptsize $\pm 0.029$} \\	
			\bottomrule
	\end{tabular}}
\end{table}		
...
Our intention here is to provide baseline experiments and compare graph kernels and GNNs. See the appendix for detailed experimental protocal. We used the following datasets, graph kernels, and GNN baselines.
...
All results can be reproduced using the scripts provided at \url{www.github.com/chrsmrrs/tudataset}.
...
We thank everybody who provided datasets for \textsc{TuDataset}.
...
\section{Experimental protocol and hyperparameters for \textsc{Zinc} and \textsc{Alchemy}}
...
For \textsc{Zinc}, we used the given train, validation split, test split, and report the MAE over the test set. For the \textsc{Alchemy} dataset, we uniformly and at random sampled 80\% of the graphs for training, and 10\% for validation and testing, respectively. Moreover, following~\cite{Che+2020,Gil+2017}, we normalized the targets of the training split to zero mean and unit variance. We used a single model to predict all targets. Following~\cite{Kli+2020}, we report mean standardized MAE and mean standardized logMAE. We repeated each experiment five times (with different random splits in case of \textsc{Alchemy}) and report average scores and standard deviations. All neural experiments were conducted on a workstation with four Nvidia Tesla V100 GPU cards with 32GB of GPU memory running Oracle Linux Server 7.7.
